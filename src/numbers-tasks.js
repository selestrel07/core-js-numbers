...
function getAverage(value1, value2) {
  ...
  if (min < 0 && max < 0) {
    return min + Math.abs(value1 - value2) / 2;
  }
  if (min < 0 && max > 0) {
...
function toNumber(value, def) {
  return ''.concat(parseInt(value, 10).toString()).valueOf() !==
    ''.concat(value)
    ? def
    : value;
}
...
function isNumber(number) {
  return (
    ...
    ''.concat((+number).toString()) === ''.concat(number.toString())
  );
}
